...
inidividual birds during a short period of time \citep{alldredge_etal:2007}. That is, we can
...
There are a few limitations to modeling behavioral effects in this
way. First, we are limited in the number of parameters we can estimate
because the \texttt{piFun} can currently only take a single argument,
\verb+p+. The approach described above makes demonstrates that we
there is much flexibility in the way that we can use \verb+p+, but it
would be better to allow \texttt{piFun} to accept multiple
arguments. This would require some modifications to
\texttt{multinomPois} and \texttt{gmultmix}, which we may do in the
future. As it currently stands, we cannot fit certain
models, such as $M_{btx}$, which includes behavioral, time, and
site-specific covariate effects
because there are not enough elements of \verb+p+ to do
so.
...
explained by site-specific covariates, time, or behavior. Currently, individual
heterogeneity attributable to animal-specific covariates cannot be explicitly modeled, although, one could
...
\]
These random effects are integrated out of the likelihod to obtain the
marginal probability of capture.

<<>>=
MhPiFun <- function(p) {
mu <- qlogis(p[1,1]) # logit(p)
...
for(i in 1:M) {
    il[i,1] <- integrate( function(x) {
        (1-plogis(mu+x))*(1-plogis(mu+x))*plogis(mu+x)*dnorm(x,0,sig)
    }, lower=-Inf, upper=Inf, stop.on.error=FALSE)$value
    il[i,2] <- integrate( function(x) {
        (1-plogis(mu+x))*plogis(mu+x)*(1-plogis(mu+x))*dnorm(x,0,sig)
    }, lower=-Inf, upper=Inf, stop.on.error=FALSE)$value
    il[i,3] <- integrate( function(x) {
        (1-plogis(mu+x))*plogis(mu+x)*plogis(mu+x)*dnorm(x,0,sig)
    }, lower=-Inf, upper=Inf, stop.on.error=FALSE)$value
    il[i,4] <- integrate( function(x) {
        plogis(mu+x)*(1-plogis(mu+x))*(1-plogis(mu+x))*dnorm(x,0,sig)
    }, lower=-Inf, upper=Inf, stop.on.error=FALSE)$value
    il[i,5] <- integrate( function(x) {
        plogis(mu+x)*(1-plogis(mu+x))*plogis(mu+x)*dnorm(x,0,sig)
    }, lower=-Inf, upper=Inf, stop.on.error=FALSE)$value
    il[i,6] <- integrate( function(x) {
        plogis(mu+x)*plogis(mu+x)*(1-plogis(mu+x))*dnorm(x,0,sig)
    }, lower=-Inf, upper=Inf, stop.on.error=FALSE)$value
    il[i,7] <- integrate( function(x) {
        plogis(mu+x)*plogis(mu+x)*plogis(mu+x)*dnorm(x,0,sig)
    }, lower=-Inf, upper=Inf, stop.on.error=FALSE)$value
}
return(il)
}
...
descriminate among these models. Thus, one might avoid model $M_h$
altogether.
...
locations, but the sample locations in the alder flycatcher dataset were
