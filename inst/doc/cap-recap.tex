...
%    they are}
  . This document focuses on the analysis of capture-recapture
  data using a class of models known as multinomial $N$-mixture
  models \citep{royle_generalized_2004, chandlerEA_2011}, which
...
{\bf X}_i' \bold \beta}$ where ${\bf X}$ is a design matrix and
${\bold \beta}$ is a vector
...
\section*{Data}
As previously mentioned, the data required by \pkg{unmarked} are an $R
\times K$
...
\begin{table}[h]
  ...
  \caption{Capture-recapture data from Table~\ref{tab:raw} in the
    format required by \pkg{unmarked}}
  \vspace{0.3cm}
...
\end{Sinput}
\end{Schunk}
Specifying the levels of \code{captureHistory} ensures that when we
...
When providing a user-defined \code{piFun}, we also need to provide
...
are affected. In \pkg{unmarked}, this can be done by supplying a
mapping-matrix to the \code{obsToY} argument in the
\code{unmarkedFrameMPois} function. \code{obsToY} needs to be a matrix
...
We are now ready to create the \code{unmarkedFrame}. In order to fit
model $M_t$, we need a covariate that references the secondary sampling
period, which we call \code{intervalMat} below. We also provide a
...
functions is evident. % \code{crPiFun} could easily be
%modified to allow for behavioral responses (trap-shyness) or for
...
\code{piFun}. This process is automated using the \code{getP}
function.
...
\code{obsToY} mapping matrix so that it has a block diagonal
...
> alfl.Hmat <- cbind(alfl.H[, , 1], alfl.H[, , 2], alfl.H[, 
     , 3])
> nVisits <- 3
> o2yGMM <- kronecker(diag(nVisits), o2y)
> umf.cr <- unmarkedFrameGMM(y = alfl.Hmat, siteCovs = alfl.covs[, 
     c("woody", "struct")], yearlySiteCovs = list(date = alfl.covs[, 
     3:5], time = alfl.covs[, 6:8]), obsCovs = list(interval = cbind(intervalMat, 
     intervalMat, intervalMat)), obsToY = o2yGMM, piFun = "crPiFun", 
     numPrimary = nVisits)
\end{Sinput}
\end{Schunk}
