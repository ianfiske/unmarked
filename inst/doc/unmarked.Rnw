...
predict(fm2, type = 'det', newdata = newData)
@
...
With binary data, observations need to be aggregated in some fashion.
See ?unmarked for an encounter history formulation. Here we use the
function fitstat1 to group the data according to the fitted values,
and compute a $\chi^2$ statistic.

<<>>=
(pb <- parboot(fm2, statistic=fitstat1, nsim=50))
@
