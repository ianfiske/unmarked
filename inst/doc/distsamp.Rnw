...
\abstract{Distance sampling is a wildlife sampling technique used to
  ...
  \citep{royle_modeling_2004}, which was developed to overcome this
...
function such as the half-normal (with shape parameter $\sigma$) over
...
units can be changed from h``a" to ``kmsq" using the \code{unitsOut}
...
the half-normal shape parameter ($\sigma$).
...
interval. \citet{chandlerEA_2011} describe a method of estimating the
