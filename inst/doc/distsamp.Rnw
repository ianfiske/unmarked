...
\abstract{Distance-sampling is a common wildlife sampling technique used to estimate population size or density. Describing how density varies spatially is often of equal interest; however, conventional methods of analysis do not allow for explicit modeling of both density and detection probability. The function distsamp implements the multinomial-Poisson mixture model of Royle et. al \citep{royle_distsamp_2004}, which was developed to overcome this limitation. This model requires that line or point transects are spatially replicated and that distance data are recorded in discrete intervals. This document describes how to format data, fit models, assess goodness-of-fit, and manipulate results. It does not cover the statistical theory and assumptions underlying distance-sampling, which the user is expected to be familiar with. }


\section{Introduction}

In distance sampling, the study design (line- or point-transect), distance class break points, transect lengths, and units of measurement need to be accounted for in the analysis. Unmarked uses an S4 class called unmarkedFrameDS to store data and metadata in a way that allows for easy data manipulation, summarization, and model specification.
...
The hist method only works for when no covariates affect the detection process, but see below for plotting covariate effects. The summary method reminds us that parameter estimates are on the natural log scale. Back-transforming them to the original scale and obtaining a standard error via the delta method is easily accomplished:
...
When covariates are present, backTransform in conjunction with linearComb can be used to for a single covariate combination. A predict method exits as well, which is especially useful when multiple covariate combinations are of interest. This method also facilitates plotting. Suppose we wanted model predictions from the covariate model along the range of covariate values studied. First we need make new data.frames holding the desired covariate combinations. Note that column names must match those of the original data, and factor variables must contain the same levels.


<<>>=
(habConstant <- data.frame(canopyHt = seq(2, 8, length=25), 
...
Several options exists to look at detection function itself, instead of it's parameters.

<<fig=TRUE>>=
x11(width=6, height=4)
par(mfrow=c(1, 2))
plot(function(x) gxhn(x, sigma=8.7), 0, 20, xlab="Distance (m)", 
	ylab="Detection probability at 2m canopy ht.")
...
legend('topright', c("CanHt=2", "Null", "CanHt=8"), 
	col=c("blue", "black", "green"), lty=1, cex=0.7)
@


Model selection and evaluating model fit can be accomplished in the same manner as for all unmarked models.
