...
\abstract{The ``{\tt un}'' in {\tt unmarked} is somewhat misleading
  because the package can be used to model spatial variation in
  abundance using data from marked animals. The three
  ...
  \code{gmultmix}. Capture-recapture models such as so-called: $M_0,
  M_t, M_b, ...$ can be fitted, but unlike other software, variation
  in abundance among ``sites'' or trap grids can also be modeled.
}
...
to estimate population size while controlling for variation in capture
...
objective of much ecological research. cite{ royle etal } proposed a
simple hierarchical model to overcome this limitation.

Before describing the multinomial $N$-mixture model of Royle et al.,
we need to first describe how capture-recapture data are collected. In
the simplest scenario, animals are captured at a single site and
released back into the population. Animals might be captured using an
array of traps or by capturing individuals by hand while searching
some well defined area. After a short time interval, the same sampling
method is repeated and a new sample of marked and unmarked individuals
are captured. If sampling stops here, Lincoln-Peterson methods can be
used to estimate population size. If additional samples are collected,
more robust methods are available. Typical data that arise from
capture-recapture studies is shown in Table XXX.
