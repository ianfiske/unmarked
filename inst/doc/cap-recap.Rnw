...
\mbox{Binomial}(N, p)$.
%When capture probability is assumed to be
...
protocol. For a capture-recapture study with 2 time periods, $K=3$
because the possibilities are $H=(11, 10, 01)$. In Equation~\ref{mod},
...
more general form of the above can be written as $\log(\lambda_i) =
{\bf X_}i' \bold \beta}$ where ${\bf X}$ is a design matrix and
...
<<fig=TRUE,include=TRUE,width=5,height=5>>=
...
\code{piFun}. This can process is automated using the \code{getP}
function.
