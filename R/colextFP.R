                   ...
                   data, type, starts, method = "BFGS", se = TRUE, ...)
{
    ...
    fc <- match.call()
    fc[[1]] <- as.name("colextFP.fit")
    formula <- list(psiformula = psiformula, gammaformula = gammaformula,
                    epsilonformula = epsilonformula, detformula = detformula,
                    fpformula = fpformula, bformula = bformula)
...
colextFP.fit <- function(formula, data, J,
                       ...
                       wts, ...)
{
    ...
    designMats <- getDesign(data,psiformula, gammaformula,
                            epsilonformula, detformula,
                            fpformula, bformula)
    V.itjk <- designMats$V
    U.itjk <- designMats$U
    S.itjk <- designMats$S
    ...
    fpParms <- colnames(U.itjk)
    bParms <- colnames(S.itjk)
...
###   theta.df <- addParm(theta.df, "detParms", nDP)
    ...
    nP <- nDP + nFP + nBP + nSP + nGP + nEP  # total number of parameters
    ...
    U.itjk[is.na(U.itjk)] <- 9999
    S.itjk[is.na(S.itjk)] <- 9999
    ...
    nll <- function(params) {
        ...
        detParams <- params[(nSP + nGP + nEP + 1) : (nSP + nGP + nEP + nDP)]
        fpParams <- params[(nSP + nGP + nEP + nDP + 1) : (nSP + nGP + nEP + nDP + nFP)]
        if (type[3]!=0) bParams <- params[(nSP + nGP + nEP + nDP + nFP + 1) : nP]
        #    psi <- plogis(params[1])
