...
simdata <- object@data[rownames(Y),]
simdata[,ynames] <- y.sim
fits[[i]] <- distsamp(stateformula, detformula, data=simdata, 
   dist.breaks=object@dist.breaks, tlength=tl, keyfun=key, survey=object@survey, 
   unitsIn=object@unitsIn, unitsOut=object@unitsOut, starts=coef(object), ...)   # define update method?
