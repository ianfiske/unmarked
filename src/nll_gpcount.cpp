...
SEXP nll_gpcount( SEXP y_, SEXP Xlam_, SEXP Xphi_, SEXP Xp_, SEXP beta_lam_, SEXP beta_phi_, SEXP beta_p_, SEXP log_alpha_, SEXP Xlam_offset_, SEXP Xphi_offset_, SEXP Xp_offset_, SEXP M_, SEXP mixture_ ) {
  arma::imat yaa = as<arma::imat>(y_); // FIXME: need to convert this to cube
  ...
  int R = yaa.n_rows; // FIXME
  int J = yaa.n_cols; // FIXME
  int T = 3; // FIXME
  ...
  phiv.reshape(T,R);
  arma::cube p(R,J,T); // FIXME!!
  arma::cube y(R,J,T); // FIXME!!
  ...
  for(int i=0; i<R; i++) {
    for(int m=0; m<lM; m++) {
      if(mixture=="P")
	f(m) = Rf_dpois(m, lam(i), false);
      else if(mixture=="NB")
        f(m) = dnbinom_mu(m, alpha, lam(i), false);
      else if(mixture=="ZIP")
	f(m) = dzip(m, lam(i), alpha);
    }
    ghi.zeros();
    for(int t=0; t<T; t++) {
      gh.zeros();
